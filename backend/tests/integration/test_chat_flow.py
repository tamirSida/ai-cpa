...
# Active, Unlimited user — assert_budget never raises; record_cost writes a harmless usage doc.
USER = User(uid="test-uid", email="owner@example.com", status="active", ai_budget_usd=None,
            created_at=now_il(), updated_at=now_il())
...
def test_followup_merge_flow(db, biz):  # doc §14.2
    ...
    first = chat_service.handle_message(db, stub, biz, USER, "main", "תוציא קבלה לנועה על עיצוב לוגו")
    assert first.action.status == "collecting_fields" and first.action.missing_fields == ["amount"]
    assert "סכום" in first.assistant_text
    second = chat_service.handle_message(db, stub, biz, USER, "main", "2800 בביט")
...
def test_query_during_pending_answers_and_reshows_question(db, biz):
    ...
    chat_service.handle_message(db, stub, biz, USER, "main", "קיבלתי 2800 מנועה על עיצוב לוגו בביט")
    res = chat_service.handle_message(db, stub, biz, USER, "main", "כמה כסף עשיתי השנה?")
...
def test_different_create_intent_supersedes(db, biz):
    ...
    old = chat_service.handle_message(db, stub, biz, USER, "main", "קיבלתי 2800 מנועה על עיצוב לוגו בביט")
    new = chat_service.handle_message(db, stub, biz, USER, "main", "יש לי לקוח חדש בשם דניאל")
...
def test_unknown_intent_and_parser_failure_fallback(db, biz):
    ...
    for text in ("בלהבלה", "עוד בלהבלה"):
        res = chat_service.handle_message(db, stub, biz, USER, "main", text)
...
def test_stale_action_expires(db, biz):
    ...
    chat_service.handle_message(db, stub, biz, USER, "main", "לקוח חדש בשם דניאל")
    acts = _actions(db, biz.id)
