...
from tests.integration.test_chat_flow import USER
...
def test_annual_report_intent_answers_with_precheck_summary(db, make_business):
    ...
    chat_service.handle_message(db, _AnnualStub(), business, USER, "main", "צור דוח שנתי")
...
def test_annual_report_intent_all_clear(db, make_business):
    biz = make_business()
    chat_service.handle_message(db, _AnnualStub(), Business.model_validate(biz), USER, "main", "דוח שנתי")
